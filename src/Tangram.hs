...
--widthToHeights :: TangramSizes -> Int -> [Int]
--widthToHeights (widthToHeightsMap, _) width = widthToHeightsMap MM.! width

--heightToWidths :: TangramSizes -> Int -> [Int]
--heightToWidths (_, heightToWidthsMap) height = heightToWidthsMap MM.! height 
...
--legalTangramSizesFromSelect :: ((SizeMap, SizeMap) -> SizeMap) -> Tangram -> Tangram -> TangramSizes
--legalTangramSizesFromSelect select left right =
--  mkTangramSizes $ addDimensions leftSelected rightSelected
-- where
--  leftSelected = select $ legalTangramSizes left
--  rightSelected = select $ legalTangramSizes right
...
---- The set of sizes a tangram can be.
--legalSizes' :: Tangram -> [(Int, Int)]
--legalSizes' image = undefined
-- where
--  minDimension = min [imageWidth image, imageHeight image]
--  maxDimension = max [imageWidth image, imageHeight image]
--  minLegalScale = fromIntegral minLargerDimension / fromIntegral maxDimension
--  maxLegalScale' = fromIntegral maxSmallerDimension / fromIntegral minDimension
--  -- We never upscale images.
--  maxLegalScale = min [maxLegalScale', 1]
